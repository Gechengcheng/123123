...
def outer_deal(df_cate, data_final_float_type):
    ...
    for i in data_final_float_type:
        ...
        print("第二次计算Q1Q3{}，{}".format(Q1, Q3))
        IQR = Q3 - Q1
        check2 = any(df_cate_copy[i] < Q1 - 1.5 * IQR)
        if check2 == True:
            print('{0}特征有低于下限的异常值'.format(i))
            alter_index = list(df_cate_copy.loc[df_cate[i] < Q1 - 1.5 * IQR].index)
...
if __name__ == "__main__":
    ...
    cate_list = list(set(df["item_first_cate_name"]))[:1]
    # 按照一级类目进行循环
    ...
    for cate in cate_list:  # 每一个一级类目拟合一个模型
        ...
        if len(cate_df) - kongzhi >= 90 and kongzhi / len(cate_df) <= 0.36 and len(
            ...
            cate_df_new = cate_df_final[cate_df_final['dt'] == '2020-11'].copy()
...
fig, ax = plt.subplots()
ax.plot(mm_1["CategoryPotential"].values[:300], "-",label="CategoryPotential")
ax.plot(mm_1["CategoryPotential_score"].values[:300], "--",label="CategoryPotential_score")
ax.legend()
#
#
#
...
# temp_mean, temp_std = np.mean(user_model.predict(X_user)), np.std(user_model.predict(X_user))
# ax[0, 1].plot(user_model.predict(X_user).apply(lambda x: ((x - temp_mean) / temp_std)).values[:100], "--",label="user_model")

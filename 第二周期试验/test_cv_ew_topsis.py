...
if __name__ == "__main__":
    ...
    file_path = r'data_25_final_index_all.txt'
    saving_path = r'./1213/0_topsis_half_0zs_25'
    ...
    for cate in cate_list:  # 每一个一级类目拟合一个模型
        ...
        if len(cate_df) - kongzhi >= 90 and kongzhi / len(cate_df) <= 0.36 and len(
            ...
            # 构建训练数据
            # Y = cate_df_train['CategoryPotential'].apply(lambda x: (1-x))
            ...
            compete_change = fit_model(X[['TopSKUBrand', 'BrandChange', 'Sale_TopSKUBrand', 'Sale_BrandChange']], Y)
            #             print(compete_model.params)
            X_change = X[['TopSKUBrand', 'BrandChange', 'Sale_TopSKUBrand', 'Sale_BrandChange']]
            with open(cate_saving_path + '_5_6整体compete_change回归模型解读.csv', 'w', encoding='utf-8') as f:
                f.write(compete_change.summary().as_csv())
            vif = get_vif(X_change)
            vif.to_csv(cate_saving_path + '_5_6compete_change共线性检验vif.csv', encoding='GBK')

            compete_cover = fit_model(X[['CR', 'CRRate', 'CR_SaleRate', 'CR_CRRate']], Y)
            #             print(compete_model.params)
            X_cover = X[['CR', 'CRRate', 'CR_SaleRate', 'CR_CRRate']]
            with open(cate_saving_path + '_5_7整体compete_cover回归模型解读.csv', 'w', encoding='utf-8') as f:
                f.write(compete_cover.summary().as_csv())
            vif = get_vif(X_cover)
            vif.to_csv(cate_saving_path + '_5_7compete_cover共线性检验vif.csv', encoding='GBK')
            ...
            with open(saving_path + r'/0_0模型解释度_R方.csv', "a", newline='', encoding='GBK') as f:
                ...
                writer.writerow((cate, 'compete_change', compete_change.rsquared_adj))
                writer.writerow((cate, 'compete_cover', compete_cover.rsquared_adj))
            ...
            list_en = ['Season', 'BrandChange', 'CRRate', 'TopSKUBrand', 'CR', 'SaleRate',
                       ...
                       'Sale_TopSKUBrand', 'Sale_BrandChange', 'CR', "TopSKUBrand", 'BrandChange'] #'CR_2', "TopSKUBrand_2", 'BrandChange_2']
            ...
            cate_df_new['CompeteScore'] = compete_model.predict(
                cate_df_new[['CR', 'CRRate', 'TopSKUBrand', 'BrandChange',
                             'CR_SaleRate', 'CR_CRRate', 'Sale_TopSKUBrand',
                             'Sale_BrandChange']]).tolist()
            # 整理最终结果并输出

...
os.chdir(r'D:\gechengcheng3\Desktop\潜力类目挖掘\编辑距离')


def damerau_levenshtein(seq1, seq2):
    """
    计算seq1,seq2的 damerau 距离，并输出结果矩阵
    ...
    for x in range(len(seq1)):
        ...
        for y in range(len(seq2)):
            ...
            # This block deals with transpositions, 两者交换
            if (x > 0 and y > 0 and seq1[x] == seq2[y - 1]
                    and seq1[x - 1] == seq2[y] and seq1[x] != seq2[y]):
                this_row[y] = min(this_row[y], two_ago[y - 2] + 1)  # 交换
...
https://blog.csdn.net/vivian_ll/article/details/93168926
矩阵回溯
"""


def get_solution(result_matrix):  # 矩阵回溯
    """
    找寻序列变化的最小编辑项
...
def search_pct(rf_df, cate, brand, dt):  # brand map code
    """
...
def changes_calculate(list1, list2, rf_df, cate, dt):
    ...
    for s in solution:
        ...
        changes_sum = changes_sum + abs(this_period - last_period)*1  # 利用占比调整编辑距离，使之更符合业务使用场景 加权编辑距离计算
    return changes_sum
...
if __name__ == "__main__":
    file_path = r'top_brand_list_org2.0.txt'
    refer_path = r'top_brand_info_org2.0.txt'  # 自己生产的
    save_path = r'top_brand_change_index_org_2.0.txt'

    list_df = pd.read_csv(file_path, delimiter='\t')  # top3 top5 top8
    ...
    for cate in all_cate_cd:
        ...
        for index, row in cate_df.iterrows():
            ...
            result_df = result_df.append({'cate': cate,  # 是不是可以直接append
                                          'dt': dt,

...
def func_sigmoid(x, low_limit, up_limit, Q3, Q1):
    # print(low_limit, up_limit, Q3, Q1)
    return low_limit + (up_limit - low_limit) * (1 / (1 + np.exp(-1 * (x - (Q3 + Q1) / 2) / 2 * (Q3 - Q1))))
...
def outer_deal(df_cate, data_final_float_type):
    ...
    """
    for i in data_final_float_type:
        Q1, Q3 = df_cate[i].quantile(q=0.25), df_cate[i].quantile(q=0.75)
        IQR = Q3 - Q1
        up_limit, low_limit = Q3 + 1.5 * IQR, Q1 - 1.5 * IQR  # 严格定义不可修改
        check = any(df_cate[i] > Q3 + 1.5 * IQR) or any(df_cate[i] < Q1 - 1.5 * IQR)  # 仅对存在异常的部分进行变换，否则保持origin
        if check is True:
            print("存在异常值的特征为{}".format(i))
            print(df_cate[i].head())
            print(low_limit, up_limit, Q3, Q1)
            df_cate[i] = df_cate[i].apply(lambda x: func_sigmoid(x, low_limit, up_limit, Q3, Q1))
            print(df_cate[i].head())
    return df_cate
...
if __name__ == "__main__":
    ...
    saving_path = r'./1215/test_sigmoid_log'  # test_sigmoid_log

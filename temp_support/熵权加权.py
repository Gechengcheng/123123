...
def outer_deal(df_cate, data_final_float_type):
    ...
    for i in data_final_float_type:
        Q1 = df_cate_copy[i].quantile(q=0.25)
        Q3 = df_cate_copy[i].quantile(q=0.75)
        IQR = Q3 - Q1
        check2 = any(df_cate_copy[i] < Q1 - 1.5 * IQR)
        if check2 == True:
            print('{0}特征有低于下限的异常值'.format(i))
            alter_index = list(df_cate_copy.loc[df_cate_copy[i] < Q1 - 1.5 * IQR].index)
...
if __name__ == "__main__":
    compare_list = []
    feature_missing = []
    file_path = r'最新主站数据提数_20201214173405.txt'
    saving_path = r'./1215/熵权法_month_07_去重_1'  # 熵权法_month_07
    ...
    for cate in cate_list:  # 每一个一级类目拟合一个模型
        ...
        if len(cate_df) - kongzhi >= 90 and kongzhi / len(cate_df) <= 0.36 and len(
            ...
            df_y_all = cate_df_final.loc[:, "growth_pct":"adjust_rate"].copy()
            df_y_all.fillna(df_y_all.mean(), inplace=True)
            ...
            # topsis得分计算
            df_y_all["growth_pct_score"] = np.abs(df_y_all["growth_pct"] - scores_dict["growth_pct"][1]) / (
                        np.abs(df_y_all["growth_pct"] - scores_dict["growth_pct"][0]) + np.abs(
                    df_y_all["growth_pct"] - scores_dict["growth_pct"][1]))
            df_y_all["adjust_rate_score"] = np.abs(df_y_all["adjust_rate"] - scores_dict["adjust_rate"][1]) / (
                        np.abs(df_y_all["adjust_rate"] - scores_dict["adjust_rate"][0]) + np.abs(
                    df_y_all["adjust_rate"] - scores_dict["adjust_rate"][1]))
            df_y_all["potential_Y"] = w_gp * df_y_all["growth_pct_score"] + w_ar * df_y_all["adjust_rate_score"]

            # 熵权加和得分排序 百分制
            df_y_vice = df_y_all.copy()
            df_y_vice.drop(["adjust_rate_score", "growth_pct_score"], axis=1, inplace=True)
            df_y_vice.reset_index(inplace=True)
            # 拿到七月份的真实值Y'
            df_y_vice_7 = df_y_vice.loc[df_y_vice["dt"] == "2020-07"].copy()
            df_y_vice_4 = df_y_vice.loc[df_y_vice["dt"] == "2020-04"].copy()
            df_y_vice = df_y_vice.loc[df_y_vice["dt"] == "2020-07"].copy()
            print(len(df_y_vice_7), len(df_y_vice_4))
            df_y_vice.set_index(['dt', 'item_last_cate_cd'], inplace=True)
            # 百分制处理
            df_y_vice = (df_y_vice - df_y_vice.min()) * 100 / (df_y_vice.max() - df_y_vice.min())
            df_y_vice_mean = df_y_vice.mean()
            df_y_vice = df_y_vice + df_y_vice_mean
            df_y_vice_max = df_y_vice.max()
            df_y_vice = df_y_vice * 100 / df_y_vice_max
            df_y_vice.sort_values(by='potential_Y', ascending=False, inplace=True)
            # 存储真实值
            df_y_vice.reset_index(inplace=True)
            df_y_vice.drop_duplicates(keep="first", inplace=True)
            df_y_vice.set_index(['dt', 'item_last_cate_cd'], inplace=True)
            df_y_vice.to_csv(cate_saving_path + r"ew_add_results.csv", index=True, header=True, encoding='GBK')

            df_y_copy = df_y_all.copy()
            with open(saving_path + r'/0_1weight_save.csv', "a", newline='', encoding='GBK') as f:
                writer = csv.writer(f)
                writer.writerow((cate, w_gp, w_ar))

            # 计算均值和标准差
            temp_mean, temp_std = np.mean(df_y_all.loc[:, "potential_Y"]), np.std(df_y_all.loc[:, "potential_Y"])
            df_y_zs = df_y_all.loc[:, "potential_Y"].apply(lambda x: ((x - temp_mean) / temp_std))

            df_y_copy.drop(["potential_Y", "growth_pct_score", "adjust_rate_score", "adjust_rate", 'growth_pct'],
                           axis=1, inplace=True)
            df_y_all = pd.concat([df_y_copy, df_y_zs], axis=1)
            df_y_all.rename(columns={"potential_Y": "CategoryPotential"}, inplace=True)
            # cate_df_final.drop_duplicates(keep="first", inplace=True)
            cate_df_final = pd.merge(cate_df_final, df_y_all, how='left', left_index=True, right_index=True,
                                     suffixes=('', '_score'))
            ...
            cate_df_new = cate_df_final[cate_df_final['dt'] == '2020-04'].copy()
            cate_df_new.drop_duplicates(keep="first", inplace=True)
...
# fig, ax = plt.subplots()
# ax.plot(mm_1["CategoryPotential"].values[:100], "-",label="CategoryPotential")
# ax.plot(mm_1["CategoryPotential_score"].values[:100], "--",label="CategoryPotential_score")
# ax.legend()
#
#
#

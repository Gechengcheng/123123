# 修缮的效果只是比原来读起来更佳易懂 不会产生歧义 修改前后的输出结果一致 只是修改之后更佳易读
...
def outer_deal(df_cate, data_final_float_type):
    ...
    for i in data_final_float_type:
        ...
        st = 1.5  # 右偏严重可能的调整
        check1, check2 = any(df_cate[i] > Q3 + 2 * st * IQR), any(df_cate[i] < Q1 - 2 * st * IQR)
        up_limit, low_limit = Q3 + 1.5 * IQR, Q1 - 1.5 * IQR  # 数学上的严格定义 不可修改

        if check1 is True and check2 is True:
            print('{0}有高于上限和低于下限异常值'.format(i))
            up_alter_index = list(df_cate.loc[df_cate[i] > Q3 + 2 * st * IQR].index)  # 拿到异常索引
            low_alter_index = list(df_cate.loc[df_cate[i] < Q1 - 2 * st * IQR].index)
            ...
            print(1.5 * IQR, Q3 + st * IQR, Q3 + 2 * st * IQR, max_up_outer_points, "\n", up_outer_ser)
            print(1.5 * IQR, Q1 - st * IQR, Q1 - 2 * st * IQR, min_low_outer_points, "\n", low_outer_ser)

            ratio_up = 1.5 * IQR / (max_up_outer_points - up_limit)
            ...
            ratio_down = 1.5 * IQR / (low_limit - min_low_outer_points)
            ...
            print('{0}有高于上限异常值 没有低于下限异常值'.format(i))
            up_alter_index = list(df_cate.loc[df_cate[i] > Q3 + 2 * st * IQR].index)  # 拿到异常索引
            up_outer_ser = df_cate.loc[up_alter_index, i].copy()  # 拿到异常序列
            max_up_outer_points = max(up_outer_ser)  # 拿到up中的max值
            print(1.5 * IQR, Q3 + st * IQR, Q3 + st * 2 * IQR, max_up_outer_points, "\n", up_outer_ser)
            ...
            min_low_outer_points = min(low_outer_ser)  # 拿到low中的min值
            print(1.5 * IQR, Q1 - st * IQR, Q1 - st * 2 * IQR, min_low_outer_points, "\n", low_outer_ser)
...
def log_x(df_cate, data_final_float_type):
    ...
    """
    for i in data_final_float_type:
        if df_cate[i].dropna().skew() >= 10 or df_cate[i].dropna().skew() <= -10:  # None会影响偏度的计算
...
if __name__ == "__main__":
    ...
    saving_path = r'./1215/test_iqr_log'
    dict_df = dict(zip(origin_df, transform_df))
    df = pd.read_csv(file_path, delimiter='\t', index_col=['dt', 'item_last_cate_cd'], encoding="utf8")
    df.drop(["uv_rate", "uv_pct"], axis=1, inplace=True)  # uv系列本身为空，底层数据加工时即为如此
    df.rename(columns=dict_df, inplace=True)  # 京喜本身没有product_id 字段
    ...
    for cate in cate_list:  # 每一个一级类目拟合一个模型
        ...
        if len(cate_df) - kongzhi >= 90 and kongzhi / len(cate_df) <= 0.4 and len(
                cate_df.dropna(axis=0, how='all')) >= 90:  # 单个特征的维度和完整记录数维度 针对京喜可以设置的稍微严格一些 主站部分可以设置的严格一些
            print("一级类目 {} 数据满足建模条件".format(cate))  # 玩具乐器的相对缺失比例过大0.38  京喜部分的处理可以设置的宽松一些
